...
int main(){
    ...
    //Fipps-Fipps
    float Q2F = 0.923;// Has a 1.03% error
    float Q4F = 0.811;// Has a 0.6% Error
    ...
    //Calculated Confidence value (99%) Critical Value for Chi-Square
    float CV95 =1.88; 
    ...
    if (infile.is_open())
        ...
        infile.close();
        std::cout << "Fipps counter starts at 2 is up until: " << count1 << std::endl;
...
count2 = count1;
    ...
    for(int k=2;k<(count1);k++) std::cout << "Normalized Peak areas: " << y[k] << "\tCorresponding errors: " << yr[k] << std::endl;

    for(int num=2;num<(count1);num++){
    ...
    std::stringstream cfilename3;
        ...
        if (cfile1.is_open())
        {
    		std::cout << "Fipps Experimental Values: " <<std::endl;
                for(int i = 2; i<count2; i++){
                    cfile1 << cos(F_angs[i-2]) << " " << y[i] << " " << yr[i] << std::endl;
                    std::cout << i << " Angle: " <<cos(F_angs[i-2]) << "Value: " << y[i]<< "Value Error: " << yr[i] <<std::endl;
    ...
    for (int J1 = 0; J1 < 5; J1++)
    {
        ...
        if (chifile.is_open())
        {
            ...
            for (delta1 = -50.000; delta1 < 50.000; delta1 += 0.010)
            {
                ...
                for(int i = 0; i<count2-2; i++) wcoeff1F +=  (y[i+2]*Wtheo[i]/pow(yr[i+2],2));
                for(int i = 0; i<count2-2; i++) wcoeff2F +=  pow(Wtheo[i]/yr[i+2],2);
                ...
                {
                	ChiSquare += pow((y[j+2]-wcoeffF*Wtheo[j])/yr[j+2],2);
		...
		wcoeff1F = 0; 
                ...
                for(int j = 0; j<loopc; j++)
                {
                	ChiSquare2 += pow((y[j+2]-wcoeffF*Wtheo[j])/yr[j+2],2);
        ...
        if (chi0file.is_open())
        {
               ...
               wcoeff1F = 0; 
                ...
                for(int i = 0; i<count2-2; i++) wcoeff1F +=  (y[i+2]*Wtheo_0[i]/pow(yr[i+2],2));
                for(int i = 0; i<count2-2; i++) wcoeff2F +=  pow(Wtheo_0[i]/yr[i+2],2);
                ...
                for(int j = 0; j<loopc; j++)
                {
                		...
                		ChiSquare_0 += pow((y[j+2]-wcoeffF*Wtheo_0[j])/yr[j+2],2);
        ...
        loopc = 0;
                ...
                for(int i = 0; i<count2-2; i++) wcoeff1F_0 +=  (y[i+2]*Wtheo_0[i]/pow(yr[i+2],2));
                for(int i = 0; i<count2-2; i++) wcoeff2F_0 +=  pow(Wtheo_0[i]/yr[i+2],2);
    ...
    if (bfilebash.is_open())
    {
        ...
        bfilebash << "title \"Angular Distribution: FIPPS-FIPPS\"" << std::endl;
        ...
        bfilebash << "s5 legend \"exp " << 1341 << " keV\"" << std::endl; //EXPERIMENT PEAK keV

        

        //------------LEGEND CONFIGURATIONS---------------|
        bfilebash << "legend 0.49, 0.57" << std::endl;
        ...
        bfilebash << "string on" << std::endl;
        bfilebash << "string loctype view" << std::endl;
        bfilebash << "string 0.48, 0.73" << std::endl;
        bfilebash << "string char size 1.3" << std::endl;
        bfilebash << "string def \"gated on 539 keV \\xg\\0 ray (Ru-100)\"" << std::endl;
    ...
    if (bfilebashChi.is_open())
    {
        ...
        bfilebashChi << "string 0.75, 0.23" << std::endl;
        bfilebashChi << "string char size 1.3" << std::endl;
        bfilebashChi << "string def \"" << 1341 << " keV \\xg\\0 ray\"" << std::endl; //PEAK GAMMA
        
        bfilebashChi << "with string" << std::endl;
        bfilebashChi << "string on" << std::endl;
        bfilebashChi << "string loctype view" << std::endl;
        bfilebashChi << "string 0.03, 0.19" << std::endl;
        bfilebashChi << "string char size 1.3" << std::endl;
        bfilebashChi << "string def \"99\%\"" << std::endl;
        
        bfilebashChi << "with string" << std::endl;
        bfilebashChi << "string on" << std::endl;
        bfilebashChi << "string loctype view" << std::endl;
        bfilebashChi << "string 0.5, 0.84" << std::endl;
        bfilebashChi << "string char size 1.3" << std::endl;
